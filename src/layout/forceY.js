...
d3.layout.forceY = function() {
  ...
  force.drag = function() {
    if (!drag) drag = d3.behavior.drag()
        .origin(d3_identity)
        .on("dragstart.forceY", d3_layout_forceDragstart)
        .on("drag.forceY", dragmove)
        .on("dragend.forceY", d3_layout_forceDragend);

    if (!arguments.length) return drag;

    this.on("mouseover.forceY", d3_layout_forceMouseover)
        .on("mouseout.forceY", d3_layout_forceMouseout)
        .call(drag);
  };
